...
const glfw = @import("glfw"); // GLFW library for windowing and input.
...
pub fn init_glfw() !void { // A ? before a type means that it can be null, useful for optional paramaters.
    ...
    // Sets up compatibility for OSX if being used. This is required to use OpenGL 4.1 on OSX, which is the latest version of OpenGL that is supported on OSX, since it is depricated there.
    if (isOSX()) {
        glfw.glfwWindowHint(glfw.GLFW_OPENGL_FORWARD_COMPAT, glfw.GLFW_TRUE); // Required for OSX to use OpenGL 4.1 or any OpenGL version above 3.2.
    }
...
pub fn createWindow(title: []const u8, width: i32, height: i32, monitor: ?*glfw.GLFWmonitor, share: ?*glfw.GLFWwindow, vsync: bool) !Window {
...
pub fn init_glad() !void {
    const loader: gl.GLADloadproc = @ptrCast(&glfw.glfwGetProcAddress); // Get the function pointer for the OpenGL function loader
    // this is required to load the OpenGL function pointers.
...
/// isOSX is a helper function to check if the current operating system is OSX, which is required for some compatibility reasons for OpenGL.
pub fn isOSX() bool {
    return builtin.os.tag == .darwin; // Returns true if the current operating system is OSX, false otherwise.
}
